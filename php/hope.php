...
<html lang="en">
  <head>
    ...
    <script>
      ...
      function delete_hope(restaurant_id){
          $.ajax({
            url: "delete_hope_love.php",
            type: "POST",
            data: { action: 'delete_hope', restaurant_id: restaurant_id },
            success: function(response){
                alert("成功刪除");
            }
          });
            ...
            <table>
                ...
                <tbody id="table-body">
                    <?php
                        ...
                        else $sql = "SELECT * FROM `hope` INNER JOIN `restaurant` ON `restaurant_id` = `id` WHERE `user_name` = '$user_name'";
                        ...
                        if ($result->num_rows > 0) {	
                            while($row = mysqli_fetch_assoc($result)){
                                ...
                                echo "<button onclick=\"insert_love('" . $row['id'] . "')\">加入最愛</button>";
                                echo "<button onclick=\"delete_hope('" . $row['id'] . "')\">刪除</button>";
                                echo "</td>";
                                echo "</tr>";
                            }
                        }
